    ...
    <div class="card-container">
      ...
      >
        ...
        <v-card-title class="d-flex">
          <h4 class="font-weight-black mt-1">{{ pet.titulo }}</h4>
  ...
  computed: {
    filteredPets() {
      return this.pets.filter(pet => {
        const nameMatch = this.name.trim().toLowerCase() === '' || pet.titulo.toLowerCase().includes(this.name.trim().toLowerCase());
  ...
  methods: {
    ...
    redirectToProfile(petId){
        // this.$router.push(`/pets/${petId}`)
         this.$router.push(`/listnews/${petId}`)
    }
  },
}
</script>
